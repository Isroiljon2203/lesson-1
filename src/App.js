...
function App() {
  ...
  return (
    ...
    <>
  <div className={dark?"container dark_theme": "container"}>
    <header>
        <h1>MakeUp Essentials</h1>
        <p>The ultimate guide to creating the ultimate makeup collection</p>
        <i onClick={() => setDark(!dark)} className={dark?"fa-solid fa-sun":"fa-solid fa-moon"} ></i>
   ...
   <main> 
    <div className="tabs">
        {
          data.map((item) => {
            return <div onClick={() => handleClick(item.id)} className='tab' key={item.id}>
              {item.name}
            </div>
          })
        }
